...
public class BlackfinPlusLoader extends AbstractProgramWrapperLoader {
  ...
  public Collection<LoadSpec> findSupportedLoadSpecs(ByteProvider provider) throws IOException {
    ...
    // Check if the file is large enough to contain at least one complete block
    // header
    if (reader.length() < 16)
      return loadSpecs;
    ...
    // Argument of the first block contains the offset of the start of the next
    // stream
    if (reader.length() < bh.argument + 16) {
      Msg.info(BlackfinPlusLoader.class, "BF Loader: not a complete boot stream.");
    }
  ...
  protected void load(ByteProvider provider, LoadSpec loadSpec, List<Option> options,
    ...
    do {
      ...
      if ((bh.byteCount != 0) && ((bh.flags & BlockHeader.BFLAG_FILL) == 0)
          && ((bh.flags & BlockHeader.BFLAG_IGNORE) == 0)) {

        long offset = reader.getPointerIndex();
        reader.setPointerIndex(offset + bh.byteCount);

        try {
          MemoryBlock mem = program.getMemory().createInitializedBlock("block" + Integer.toString(i), target,
              provider.getInputStream(offset), bh.byteCount, monitor, false);
          ...
          try {
            MemoryBlock mem = program.getMemory().createInitializedBlock("block" + Integer.toString(i), target,
                provider.getInputStream(offset), bh.byteCount, monitor, true);
            mem.setPermissions(true, true, true);
            mem.setComment(bh.getComment());
          } catch (Exception f) {
      ...
      if ((bh.flags & BlockHeader.BFLAG_FILL) != 0) {
        try {
          MemoryBlock mem = program.getMemory().createInitializedBlock("block" + Integer.toString(i), target,
              bh.byteCount, (byte) 0, monitor, false);
          mem.setPermissions(true, true, true);
          mem.setComment(bh.getComment());

          if (bh.argument != 0) {
            Address pos = target;
            byte[] b = { (byte) bh.argument, (byte) (bh.argument >> 8), (byte) (bh.argument >> 16),
                (byte) (bh.argument >> 24) };
